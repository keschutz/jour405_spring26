...
Voter turnout and unemployment seem to have the strongest negative correleation and a linear relationship. Voter turnout and race appears to have the strongest positive correlation, but it still has a weak linear relationship. Based on the charts, there doesn't seem to be much correlation specifically between voter turnout and population, or median age, but generally there seems to be minimal correlation between voter turnout for most variables aside from unemployment or race.
...
The model's mutliple r-squared value of 0.8539 means roughtly 85% of the variability in voter turnout is explained by the model, which is quite high. The residual standard error is relatively low at 0.02174, suggesting the model doesn't deviate much from actual voter turnout. The F-statistic p-value is also far under 0.05, meaning the model as a whole is statistically significant. This indicates that the model should be useful in predicting voter turnout.

However, certain individual variables in the model do not have promising results. Income, median age and race are the only variables with -values less that 0.05, showing statistically significant relationships with voter turnout.
...
I created three alternative models using income, unemployment and median age. Model 1 includes all the variables and has an r-squared value of 0.8, meaning 80% of the variability in voter turnout can be explained by the variables. For models 2-4, the r-squared value is significantly lower. However, the r-squared for unemployment is significantly than income and median age, at around 0.525.
...
# Model 3
model3 <- lm(turnout ~ pct_unemployed, data = md_turnout)
summary(model3)

# Model 4
model4 <- lm(turnout ~ median_age, data = md_turnout)
summary(model4)
...
I would use this data to understand and convey to readers what variables may play bigger roles in people deciding to go out and vote. It also allows me to understand the different size of impact each variable may have on voting turnout, which would be helpful in reporting about who may be most likely to vote. I would also want to get interviews from people in different points of the spectrum in all of these groups, ideally seeing if these predictions hold true with real voters.
