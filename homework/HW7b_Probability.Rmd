...
wh_paid |> summarize(mean_salary = mean(SALARY),
...
The distribution is not normal. Rather, it appears skewed to the right. Many of the salaries cluster around $110000.
...
The actual percentages of data within the first and second standard deviations from the mean are both less than the expected percentage, so we know that the data does not follow the empirical rule. Given that 100% of the data falls within three standard deviations from the mean, it is unlikely that there are outliers.
...
If the data were normally distributed, we would expect 16.88% of the salaries to be above $150,000. According to the actual data, 18.18% of the salaries are aboce $150,000.
...
This tells us that 32.58% of the salaries are between $80000 and $120000. This also confirms that the data is not normally distributed, because the actual range percentage does not match what would be expected of a normal distribution.
...
The biggest difference between the observed count and expected count was in the $60k-$90k category, because so many salaries for White House positions fall within this range, according to the data set. There are far more people bringing in salaries within that range than would be expected of a normal distribution.
...
HL: Wage gaps in the White House

While over 81% of White House employees bring in under $150,000 each year, some salaries range up to $225,700, according to salary data.

I would like to compare White House salary data to other federal worker salary data, maybe even with other countries' federal governments to see how ours compares. It would also be helpful to compare the positions of people in the White House who are making these substantially higher salaries to those with salaries of $150,000 and under to see what the disconnect is.
