...
There seems to be a slight curve in the data, with a slight bump up in the summer months and dips on either end in the winter months, so you could draw a slightly curved horizontal line through the approximate middle of the data. I would also calculate the upper and lower boundaries for outliers and draw horizontal lines at those places in order to identify the outliers in the data, which could reveal a pattern in when call numbers are particularly high or particularly low. From my observation, the highest spikes in call numbers are distributed throughout the year, although the winter months appear to have less severe spikes and generally lower call numbers with the exception of New Year's Day.
...
The identified points are generally similar to what I noted in Task 3. Unusually high points are not confined to one period, but unusually low days are more populous in the colder months, with New Year's Day being a notably high point for the winter.
...
Tuesday, Oct. 4 saw 23 calls, which is more than at least 95% of all other recorded days in the data set.
...
As mentioned in the previous task, I would check if any of the days with particularly high call numbers coincided with some kind of event on the same day. It would probably also be helpful to cross-reference with the police department to see if they are aware of any kind of event that would cause that spike. It may also be interesting to see if we can find any kind of reasoning as to why many of the low spikes are over the winter, but I don't think that would be easily deciphered because it likely wouldn't be the result of some kind of event.
