...
There does not appear to be any relationship between the number of fouls and the goal differentials based on the scatterplot. The data points are very spread out and in sort of a blob, and the line across the scatterplot is basically flat, which constitutes no relationship between the variables.

### Task 7: Choose another variable and repeat the stpes in Tasks 4-5, then describe the result.
...
```
There is a positive correlation between shot attempts and goal differentials based on the scatterplot. Logically, this makes sense, because you need to have shot attempts in order to score goals. However, just because we can see this correlation doesn't mean that there aren't other variables that also contribute to the goal differentials.
