...
```{r}
murders <- read_csv("https://raw.githubusercontent.com/dwillis/jour405/refs/heads/main/data/pg_murder_temps.csv")
```
...
```{r}
murders |> summarize(correlation = cor(murders, temperature, method="pearson"))
```
...
The correlation calculation is 0.5825525, which I would say is moderately strong, and reflected by the scatterplot. Based on the scatterplot, we can see a positive correlation between temperatures and murders, suggesting that the temperature may play a role in murder numbers. However, there are likely other variables involved.
